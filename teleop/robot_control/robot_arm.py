...
class G1_29_ArmController:
    ...
    def __init__(self, motion_mode = False, simulation_mode = False):
        ...
        # # 将目标位置初始化为当前位置
        # current_dual_arm_q = self.all_motor_q[15:29]
        # current_dual_leg_q = self.all_motor_q[:15]
        # self.q_target = np.concatenate([current_dual_leg_q,current_dual_arm_q])
        print(self.all_motor_q[15:29])
        self.q_target = np.array([ 0.29114 , 0.2156 , -0.03136 , 0.9844 , 0.02665 , 0.06433 , -0.00199 , 0.29294 , -0.21333 , 0.0316 , 0.98565 , -0.15575 , 0.01917 , 0.00133])
    ...
    def _ctrl_motor_state(self):
        ...
        while True:
            ...
            # 线程安全地读取目标值
            with self.ctrl_lock:
                # arm_q_target     = self.q_target[15:29]
                # arm_tauff_target = self.tauff_target
                # leg_q_target     = self.q_target[:15]
                arm_q_target     = self.q_target
                arm_tauff_target = self.tauff_target
            ...
            #     self.msg.motor_cmd[id].q = leg_q_target[idx]    # 目标位置
            #     # self.msg.motor_cmd[id].dq = 0                          # 目标速度（0表示位置控制）
            #     # self.msg.motor_cmd[id].tau = arm_tauff_target[idx]    # 前馈力矩
