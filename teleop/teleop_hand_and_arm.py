...
def on_press(key):
    global STOP, START, RECORD_TOGGLE,SUB_TASK_INDEX
    if key == 'r':
        START = True
    elif key == 'q':
        START = False
...
if __name__ == '__main__':
    ...
    if args.ee == "dex3":
        ...
        left_hand_pos_array = Array('d',6, lock = True)      # [input]
        right_hand_pos_array = Array('d', 6, lock = True)     # [input]
        dual_hand_data_lock = Lock()
        dual_hand_state_array = Array('d', 12, lock = False)   # [output] current left, right hand state(12) data.
        dual_hand_action_array = Array('d', 12, lock = False)  # [output] current left, right hand action(12) dat
        hand_ctrl = Brainco_Controller(left_hand_pos_array, right_hand_pos_array, dual_hand_data_lock, dual_hand_state_array, dual_hand_action_array, simulation_mode=args.sim)
    ...
    #         left_hand_pos_array[:] = np_array
    #     with right_hand_pos_array.get_lock():
    #         right_hand_pos_array[:] = np_array
    #     time.sleep(1)

    #     np_array = np.array([1.52, 0, 0, 1.47, 1.47, 1.47])
    #     with left_hand_pos_array.get_lock():
    #         left_hand_pos_array[:] = np_array
    #     with right_hand_pos_array.get_lock():
    #         right_hand_pos_array[:] = np_array
    ...
    while not STOP:
        ...
        elif args.ee == "brainco" and args.xr_mode == "controller":
            # print("aButten: ",tele_data.tele_state.right_aButton)
        ...
        if args.record:
            ...
            if args.ee == "dex3" and args.xr_mode == "hand":
                with dual_hand_data_lock:
                    ...
                    # print("current_body_state:",current_body_state)
                    current_body_action = []
            ...
            if RECORD_RUNNING:
                ...
                if args.sim:
                    ...
                    recorder.add_item(colors=colors, depths=depths, states=states, actions=actions,sub_index=sub_index)
